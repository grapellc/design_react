...
3. `doc-gen:file` 경로와 import 경로(`@/components/ui/*` vs `@grape_design_react/react`)를 검증합니다.
...
- **Snippet 없음** → `@grape_design_react/react`에서 직접 import, Usage만 작성
...
**Composite 컴포넌트인 경우**:
- Snippet 파일 또는 `@grape_design_react/react` export를 확인하여 sub-component 목록 추출
...
```package-install
npx @grape_design_react/cli@latest add ui:{component-id}
```
...
**Snippet 레이어가 없는 경우**:
- Installation 섹션 생략
- Usage 섹션에서 바로 `@grape_design_react/react`에서 import 안내
...
// 또는
// } from "@grape_design_react/react"  // Snippet이 없는 경우
```
...
1. **Import 경로 명확히**:
   ...
   - Snippet 없음: `@grape_design_react/react`
...
- [ ] Import 경로가 정확한가? (`@/components/ui/*` vs `@grape_design_react/react`)
...
1. **Snippet vs Direct Import**:
   ...
   - Snippet이 없으면 `@grape_design_react/react`에서 직접 사용
