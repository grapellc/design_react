...
- 6d30b72: Stackflow와 함께 AppScreen 사용 시 최상위 AppScreen이 push/pop될 때, 이외의 AppScreen이 고유한 `transitionStyle`을 재생하는 문제를 수정합니다. 같은 스택 내에 여러 `transitionStyle`이 공존할 때 자연스러운 트랜지션을 제공하기 위해 최상위 AppScreen의 `transitionStyle`을 재생합니다. ([데모](https://grape_design_react.io/react/stackflow/app-screen#transition-styles))
...
- ea488c5: @grape_design_react/stackflow: @stackflow/react-ui-core를 peerDependencies에서 dependencies로 이동합니다.
...
- 7529e31: @grape_design_react/stackflow 패키지에서 `useActivityZIndexBase` 훅을 제공합니다.
...
- cdc0930: `@grape_design_react/stackflow` 백스와이프 애니메이션 개선
