...
```package-install
npx @grape_design_react/cli@latest add ui:list
```
...
@grape_design_react/react 패키지에서 제공하는 `List`와 `ListHeader` 컴포넌트는 다음과 같은 구조로 사용됩니다.
...
```tsx
import { ListHeader, List, Icon } from "@grape_design_react/react";
...
```tsx
import { List as SeedList } from "@grape_design_react/react";
...
```tsx
import { ListCheckItem } from "grape_design_react/ui/list";
