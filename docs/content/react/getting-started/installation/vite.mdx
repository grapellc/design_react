...
<Callout>
  위에서 설정한 [grape_design_react.json
...
```package-install
npm install -D @grape_design_react/vite-plugin vite-tsconfig-paths
```
...
import { seedDesignPlugin } from "@grape_design_react/vite-plugin"; // [!code highlight]
