...
위에서 설정한 [grape_design_react.json path](/react/getting-started/cli/configuration#path)의 경로와 동일해야 해요.
...
```package-install
npm install -D @grape_design_react/rsbuild-plugin
```
...
import { seedDesignPlugin } from "@grape_design_react/rsbuild-plugin"; // [!code highlight]
