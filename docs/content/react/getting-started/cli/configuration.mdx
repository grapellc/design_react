---
title: Configuration
description: grape_design_react.json 설정을 안내해요.
---


## grape_design_react.json 만들기

`grape_design_react.json` 파일은 CLI로 생성하거나 직접 추가할 수 있어요.
...
CLI로 grape_design_react을 추가하는 방법을 알아봐요.
...
<Callout>
`grape_design_react.json`은 엄격하게 파싱돼요. 문서에 없는 키를 임의로 추가하면 CLI 실행 시 오류가 날 수 있어요.
</Callout>
...
| 옵션 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `path` | `string` | 없음 (init에서는 `./grape_design_react`) | 스니펫이 생성될 루트 경로 |
...
<Files>
  <Folder name="grape_design_react (path)" defaultOpen>
...
3. `grape_design_react.json`의 `telemetry`
4. 기본값 `true`
